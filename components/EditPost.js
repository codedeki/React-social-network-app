...
function EditPost(props) {
        ...
        try {
          ...
          if (response.data) {
            ...
            if (appState.user.username !== response.data.author.username) {
              appDispatch({type: "flashMessage", value: "You do not have permission to perform that action."})
  ...
  useEffect(()=> {
    if (state.sendCount) {
      ...
      const ourRequest = axios.CancelToken.source();
      const postUrl = `/post/${state.id}/edit`;

      async function fetchPost() {
        try {
          const response = await axios.post(postUrl, { title: state.title.value, body: state.body.value, token: appState.user.token }, { cancelToken: ourRequest.token })
